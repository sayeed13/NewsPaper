        ...
        <div class="w-full overflow-x-auto">
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    @php( $i = 1)
                    @foreach ($subcategory as $adminsubcat)
                    <tr class="text-gray-700 dark:text-gray-400">
                        <td class="px-4 py-3">
                            <span>{{ $i++ }}</span>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <p class="font-semibold">{{ $adminsubcat->name }}</p>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <p class="font-semibold">{{ $adminsubcat->cat_name }}</p>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-4 text-sm">
                                <a href="{{ route('edit.subcategory', $adminsubcat->id) }}">
                                    <button
                                ...
                                <a href="{{ route('delete.subcategory', $adminsubcat->id) }}"
